...
// Read and register each API function as an Express route
fs.readdirSync(dirPath, { withFileTypes: true })
  .filter(dirent => dirent.isFile() && dirent.name.endsWith('.js'))
  .forEach(dirent => {
    const routeName = dirent.name.split('.')[0];
...
// Handle API routes separately (so they're not affected by SPA fallback)
app.use('/api', (req, res, next) => {
  res.status(404).json({ error: 'API route not found' });
});

// Handle frontend SPA routing (React Router)
app.use(historyApiFallback({
  index: '/index.html', // Ensures React app gets served for unknown routes
  rewrites: [{ from: /^\/api\/.*$/, to: (context) => context.parsedUrl.path }],
}));

// Serve up the GUI - if build dir exists, and GUI feature is enabled
if (process.env.DISABLE_GUI && process.env.DISABLE_GUI !== 'false') {
  app.get('*', async (req, res) => {
    const placeholderContent = await fs.promises.readFile(placeholderFilePath, 'utf-8');
    const htmlContent = placeholderContent.replace(
...
// Custom 404 page for unmatched API routes
app.use('/api/*', (req, res) => {
  res.status(404).json({ error: 'API route not found' });
});
